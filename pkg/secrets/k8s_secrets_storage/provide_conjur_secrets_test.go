...
func TestProvideConjurSecrets(t *testing.T) {
	...
	Convey("RetrieveRequiredK8sSecrets", t, func() {
		...
		Convey("Given a k8s secret with an invalid 'conjur-map'", func() {
			...
			mocks.MockK8sDB["invalid_conjur_map_secret"] = mocks.MockK8sSecret{
				Data: secretData,
			}
	...
	Convey("run", t, func() {
		...
		Convey("Given a k8s secret which is mapped to a conjur secret with an empty secret value", func() {
			...
			addK8sSecretToMockDB("k8s_secret3", "conjur_variable3")
			requiredSecrets := []string{"k8s_secret3"}
...
func prepareMockDBs() {
	...
	mocks.MockConjurDB["conjur_variable3"] = []byte("")
}
