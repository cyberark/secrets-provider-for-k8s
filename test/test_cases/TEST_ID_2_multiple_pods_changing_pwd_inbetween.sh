...
wait_for_it 30 "$cli exec -n $TEST_APP_NAMESPACE_NAME ${pod_name1} printenv | grep TEST_SECRET | cut -d \"=\" -f 2 | grep 'supersecret'"
...
wait_for_it 30 "$cli exec -n $TEST_APP_NAMESPACE_NAME ${pod_name2} printenv | grep TEST_SECRET | cut -d \"=\" -f 2 | grep 'secret2'"
...
wait_for_it 30 "$cli get pods | grep test-env | grep Running | wc -l | tr -d ' ' | grep '^3$'"
...
do
     ...
     wait_for_it 30 "$cli exec -n $TEST_APP_NAMESPACE_NAME ${new_pod} printenv | grep TEST_SECRET | cut -d \"=\" -f 2 | grep 'secret3'"
done
