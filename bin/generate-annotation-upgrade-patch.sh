  ...
  echo "${sp_init_container}" | \
    jq \
      ...
      '
        .value.env |
        to_entries |
        map(select(.value.name == ($SP_ENV_VARS | split(",") | .[])))
      '
}
...
function update_k8s_secrets_annot_format() {
  local sp_annots="$1"
...
function append_secrets_volume_remove_op_to_patch() {
  ...
  echo "${deployment_manifest_json}" | \
    jq \
      ...
      '
        .spec.template.spec.volumes // [] |
        map(select((.secret.secretName // "") == ($k8s_secrets | split(",")  | .[]))) |
        length > 0
      '
}
...
function main() {
  ...
  sp_annots="$(update_k8s_secrets_annot_format "${sp_annots}")"
